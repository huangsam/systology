...
description: "Multimodal video feature extraction: Apple-native (Vision, AVFoundation) vs. OpenCV/pybind11."
...
- **Bottlenecks & Limitations:** Video decoding is CPU/IO-bound but has been optimized by implementing single-pass metric aggregation and caching. Remaining limitations include the need for per-dataset calibration for heuristic thresholds (e.g. blur or scene changes) and the necessity of static linking/OpenCV version pinning to maintain cross-platform ABI compatibility.
