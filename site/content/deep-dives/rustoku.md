...
- **Web Demo Enhancements:** The interactive demo (v0.15.0) received major UX upgrades. A new **visual step trace** mode (`demo/src/trace.ts`) animates each solver step directly on the grid—highlighting the current cell, candidate eliminations, and the applied technique—giving users a frame-by-frame view of the solve path. Undo/redo history is now fully validated: duplicate board states are rejected on push and guarded against off-by-one edge cases at startup. The new-game modal was upgraded with proper ARIA roles and focus management, and a dedicated `demo/src/modal.ts` module was extracted to own that lifecycle.
- **Workspace Build Configuration:** A `.cargo/config.toml` was added to the repository root to standardize cross-crate build targets and linker flags for the entire Cargo workspace, ensuring WASM and native builds share consistent toolchain settings without per-crate repetition.
