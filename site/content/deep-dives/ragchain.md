...
**Problem:** Balancing precision and latency for hybrid retrieval (BM25 lexical search + semantic vector search) while keeping everything local introduces overlapping trade-offs: index refresh latency vs. staleness, embedding model choice vs. memory footprint, fusion parameter tuning vs. query diversity, and evaluation rigor vs. iteration speed. A RAG system that retrieves the wrong documents generates confidently wrong answers.
