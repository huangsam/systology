...
- **LLM-as-a-Judge Evaluation:** An automated evaluation module (`src/ragchain/evaluation/judge.py`) benchmarks RAG output quality across three core metrics—**correctness**, **relevance**, and **faithfulness**—scored on a 1–5 scale. The evaluator features robust nested JSON extraction to parse LLM outputs, score boundary validation, and execution timing metrics (`@timed`), ensuring reproducible batch evaluation without manual review.
