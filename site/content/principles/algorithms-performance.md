...
tags: [algorithms, latency, optimization, performance, search-algorithms]
...
**Anti-pattern — Parallelism Without Measurement:** Spinning up 64 threads because "more is faster." Thread contention, context switching, and memory overhead can make parallel code slower than sequential. Benchmark with varying worker counts and find the knee of the curve. See the [Service Resilience]({{< ref "/principles/service-resilience" >}}) principles for a production-grade approach to worker scaling and background job queues.
